...
export function DriversTab({ drivers, isLoading, addDriver, fetchDrivers }: DriversTabProps) {
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between gap-2 mb-1">
                                            <h3 className="text-sm font-black truncate">{driver.name}</h3>
                                            <Badge variant={driver.isAvailable ? "outline" : "secondary"} className="text-[9px] uppercase font-black px-1.5 h-4 border-emerald-500/20 text-emerald-600 bg-emerald-500/5">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-[10px] font-bold text-muted-foreground">{driver.licenseType || "Cat. B"}</span>
                                            </div>
                                            <div className="flex items-center gap-1.5">
                                                <Clock className="h-3 w-3 text-primary/60" />
                                                <span className="text-[10px] font-bold text-muted-foreground">{driver.onTimeDeliveryRate}% On-Time</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {driver.currentVehicleId && (
                                    <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-black uppercase text-muted-foreground">{driver.currentVehicleId}</span>
                                        </div>
                                        <Button size="sm" variant="ghost" className="h-6 text-[9px] font-black uppercase tracking-widest text-primary px-2">
                                            Detallado
                                        </Button>
                                    </div>
                                )}
