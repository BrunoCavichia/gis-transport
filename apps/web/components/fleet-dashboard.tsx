...
import type { FleetVehicle, MovementState, VehicleMetrics, FleetJob } from "@gis/shared";
...
export function FleetDashboard({
    ...
    const kpis = useMemo(() => {
        ...
        const batteryVehicles = metricsArray.filter((m) => m.batteryLevel !== undefined);
        const activeCount = metricsArray.filter((m) => m.status === "active").length;

        return {
            activeVehicles: activeCount,
            avgFuel: fuelVehicles.length
                ? Math.round(fuelVehicles.reduce((sum, m) => sum + (m.fuelLevel || 0), 0) / fuelVehicles.length)
                : null,
            avgBattery: batteryVehicles.length
                ? Math.round(batteryVehicles.reduce((sum, m) => sum + (m.batteryLevel || 0), 0) / batteryVehicles.length)
                : null,
            totalDistance: Math.round(metricsArray.reduce((sum, m) => sum + m.distanceTotal, 0)),
        };
    }, [vehicles]);
            ...
            {selectedVehicle ? (
                ...
                <div className="flex flex-col min-h-0">
                    ...
                    <div className="px-4 py-3 shrink-0 flex items-center justify-between border-b border-border/10 bg-background/50 backdrop-blur-sm">
                        <div>
                            <h2 className="text-sm font-bold text-foreground tracking-tight">Flota Activa</h2>
                            <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5">
                                <Activity className="h-3 w-3 text-green-500" />
                                <span>{kpis.activeVehicles} de {vehicles.length} en servicio</span>
                            </div>
                        </div>
                        <div className="text-right">
                            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60">Energía de Flota</div>
                    ...
                    <div className="flex-1 overflow-y-auto min-h-0 p-3 pt-0 space-y-2.5">
                        {vehicles.map((vehicle) => {
                            ...
                            const isElectric = vehicle.type.id.includes("electric") || vehicle.type.id === "zero" || (m?.batteryLevel !== undefined && m?.fuelLevel === undefined);
                                    ...
                                    <div className="flex items-center gap-3 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold text-sm text-foreground truncate">{vehicle.label}</span>
                                                <span className={cn(
                                                    "inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-medium border shadow-[0_1px_2px_rgba(0,0,0,0.05)]",
                                                    movement === "on_route" ? "bg-emerald-50 text-emerald-700 border-emerald-100" :
                                                        movement === "moving" ? "bg-blue-50 text-blue-700 border-blue-100" :
                                                            "bg-zinc-50 text-zinc-600 border-zinc-100"
                                                )}>
                                                    {movement === "on_route" ? "En Ruta" : movement === "moving" ? "Moviendo" : "Parado"}
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5">
                                                ...
                                                <div className="flex items-center gap-1.5 text-[10px] font-medium">
                                                    {m.speed > 0 && (
                                                        <span className={cn(
                                                            ...
                                                            m.maxSpeed && m.speed > m.maxSpeed ? "text-red-600 font-bold animate-pulse" : "text-foreground"
                                                        )}>
                                                            {m.speed}
                                                            <span className="text-[8px] ml-0.5 opacity-50">km/h</span>
                                                        </span>
                                                    )}
                                                    {m.maxSpeed && (
                                                        <span className="bg-zinc-100 text-zinc-500 px-1 rounded-[4px] border border-zinc-200/50 text-[8px] flex items-center gap-0.5" title="Límite de la vía">
                                                            <span className="opacity-40 tracking-tighter">L</span>{m.maxSpeed}
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    {energyLevel !== undefined && (
                                                        <div className="flex items-center gap-1 bg-muted/50 px-1.5 py-0.5 rounded-md border border-border/50">
                                                            {isElectric ? <Battery className="h-2.5 w-2.5 text-blue-500" /> : <Fuel className="h-2.5 w-2.5 text-orange-500" />}
                                                            <span className={cn("text-[10px] font-mono font-medium", energyLevel < 20 ? "text-red-600" : "text-foreground")}>
                                                                {energyLevel}%
                                                            </span>
                                                        </div>
                                                    )}
                        ...
                        {vehicles.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                                ...
                                <span className="text-xs font-medium opacity-50">No hay vehículos conectados</span>
                            </div>
                        )}
                    </div>
                </div>
