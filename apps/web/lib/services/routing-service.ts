...
export class RoutingService {
    ...
    static async optimize(
        ...
        // 1. Snapping
        const allCoords = [...vehicles.map(v => v.position), ...jobs.map(j => j.position)];
    ...
    private static processUnassignedJobs(vroomResult: VroomResult, jobs: FleetJob[], vehicleOffset: number) {
        ...
        vehicles.forEach((v, idx) => {
            ...
            if (jobSteps.length === 0 && profile.avoidPolygons.length > 0) {
                const forbiddenJobNames = jobs
                    .filter((job) => profile.avoidPolygons.some((poly) => this.isPointInPolygon(job.position, poly)))
                    .map((job) => job.label);
    ...
    private static async fetchOrsRoute(
        ...
        try {
            ...
            return {
                vehicleId,
                coordinates: feat.geometry.coordinates.map(([lon, lat]: [number, number]) => [lat, lon]),
