...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[API Zones] ${lat},${lon} R:${radius} => Found ${zones.length} zones`);
    if (zones.length > 0) {
       const z = zones[0];
       const depth = Array.isArray(z.coordinates) && z.coordinates.length > 0 
          ? (Array.isArray(z.coordinates[0]) ? (Array.isArray(z.coordinates[0][0]) ? (Array.isArray(z.coordinates[0][0][0]) ? "4D" : "3D") : "2D") : "1D")
          : "empty";
       console.log(`[API Zones] Sample: ${z.name}, type: ${z.type}, coords depth: ${depth}`);
    }
