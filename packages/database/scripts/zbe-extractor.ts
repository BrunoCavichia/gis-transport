...
const MAJOR_CITIES = [
    ...
    "Santander", "Castellón de la Plana", "Logroño", "Badajoz", "Salamanca"
];
...
async function queryOverpass(city: string, retryCount = 0): Promise<OverpassElement[]> {
    ...
    try {
        ...
        if (retryCount < 3) {
            ...
            console.warn(` Error querying ${city} (${error.message}). Retrying in ${delay / 1000}s... (Attempt ${retryCount + 1}/3)`);
...
async function extractZBE() {
    ...
    try {
        ...
        console.warn(` National query failed (${error.message}). Switching to city-by-city backup...`);
...
async function saveElements(elements: OverpassElement[], defaultCity?: string): Promise<number> {
    ...
    for (const element of elements) {
        ...
        try {
            ...
            console.error(`   Failed to save zone ${zoneData.osmId}:`, (error as any).message);
            continue;
        }
    }
    return count;
...
function processElement(element: OverpassElement) {
    ...
    const tags = element.tags || {};
    let type = "RESTRICTED";
    if (tags.boundary === "low_emission_zone" || tags["zone:traffic"] === "environmental" || tags.hazard === "low_emission_zone") type = "LEZ";
    ...
    return {
        ...
        name: tags.name || tags.official_name || tags.description || "ZBE " + (tags.city || "España") + " " + element.id,
        type,
        geometry: coords,
